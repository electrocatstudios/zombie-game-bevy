...
impl Plugin for GamePlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(Update, (menu_return_check,game_update,zombie_mover).run_if(in_state(MainGameState::Game)))
...
fn game_setup(
    ...
    //Zombie
    {
        ...
        // locations.push(Vec2::new(-350.0, 0.0)); // This doesn't work with the angle 
        locations.push(Vec2::new(-300.0, 200.0));
        locations.push(Vec2::new(300.0, 200.0));
        locations.push(Vec2::new(300.0, -200.0));
    ...
    // Player
    {
        ...
        commands.spawn((
            SpriteSheetBundle {
                texture_atlas: texture_atlas_handle,
                sprite: TextureAtlasSprite::new(animation_indices.first),
                transform: Transform::from_xyz(80.0, 50.0, 0.0).with_scale(Vec3::splat(0.5)),
                ..default()
            },
            animation_indices,
            AnimationTimer(Timer::from_seconds(0.1, TimerMode::Repeating)),
    ...
    for (mut zombie, mut transform) in &mut zombies {
        ...
        if cur_loc.x != transform.translation.x {
            if (cur_loc.x - transform.translation.x).abs() <= ZOMBIE_SPEED * time.delta_seconds() {
                ...
                if transform.translation.x > cur_loc.x {
        ...
        if cur_loc.y != transform.translation.y {
            if (cur_loc.y - transform.translation.y).abs() <= ZOMBIE_SPEED * time.delta_seconds() {
                ...
                if transform.translation.y > cur_loc.y {
        ...
        if diff.y == 0.0 {
            ...
            angle = (diff.y/diff.x).tan() + std::f32::consts::PI;
            println!("pos {:?}, target: {:?}, diff: {:?}, angle: {:?} ", pos, target, diff, angle);
        }
